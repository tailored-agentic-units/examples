...
func main() {
	...
	baseConfig, err := agentconfig.LoadAgentConfig("orchestrate/phase-07-conditional-routing/config.ollama.json")
	if err != nil {
...
Start response with "APPROVE:" or "REJECT:" followed by reasoning.`,
	...
	techAnalyst, err := agentutil.NewAgent(techAnalystCfg)
	if err != nil {
		log.Fatalf("Failed to create technical-analyst: %v", err)
	}

	secAnalyst, err := agentutil.NewAgent(securityAnalystCfg)
	if err != nil {
		log.Fatalf("Failed to create security-analyst: %v", err)
	}

	bizAnalyst, err := agentutil.NewAgent(businessAnalystCfg)
	if err != nil {
		log.Fatalf("Failed to create business-analyst: %v", err)
	}

	reviewer1, err := agentutil.NewAgent(reviewer1Cfg)
	if err != nil {
		log.Fatalf("Failed to create reviewer-alpha: %v", err)
	}

	reviewer2, err := agentutil.NewAgent(reviewer2Cfg)
	if err != nil {
		log.Fatalf("Failed to create reviewer-beta: %v", err)
	}

	reviewer3, err := agentutil.NewAgent(reviewer3Cfg)
	if err != nil {
	...
	analysisAgents := []struct {
		...
		{"technical-analyst", techAnalyst, techAnalystCfg, "Technical"},
		{"security-analyst", secAnalyst, securityAnalystCfg, "Security"},
		{"business-analyst", bizAnalyst, businessAnalystCfg, "Business"},
	}
	...
	analyzeProcessor := func(ctx context.Context, item struct {
		...
		messages := agentutil.BuildMessages(item.cfg, prompt)
	...
	reviewAgents := []struct {
		...
		{"reviewer-alpha", reviewer1, reviewer1Cfg},
		{"reviewer-beta", reviewer2, reviewer2Cfg},
		{"reviewer-gamma", reviewer3, reviewer3Cfg},
	}
