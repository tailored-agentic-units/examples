...
func main() {
	...
	baseConfig, err := agentconfig.LoadAgentConfig("orchestrate/phase-05-parallel-execution/config.ollama.json")
	if err != nil {
		log.Fatalf("Failed to load config: %v", err)
	}

	baseConfig.Name = "sentiment-analyst"
	baseConfig.SystemPrompt = `You are an expert sentiment analysis system.
...
Respond in format: "SENTIMENT" where SENTIMENT is one word: positive, neutral, or negative.`
	...
	sentimentAgent, err := agentutil.NewAgent(baseConfig)
	if err != nil {
