...
import (
	...
	"github.com/tailored-agentic-units/orchestrate/workflows"
...
Provide your response in your directed JSON format.`,
		...
		messages := agentutil.BuildMessages(registry.ResearchDirectorCfg, prompt)

		resp, err := registry.ResearchDirector.Chat(ctx, messages)
		if err != nil {
			return s, fmt.Errorf("research director failed: %w", err)
		}

		request, err := parseJSON[ProcurementRequest](resp.Text())
		if err != nil {
...
Provide your response in your directed JSON format.`,
		...
		messages := agentutil.BuildMessages(registry.CostAnalystCfg, prompt)

		resp, err := registry.CostAnalyst.Chat(ctx, messages)
		if err != nil {
			return s, fmt.Errorf("cost analyst failed: %w", err)
		}

		analysis, err := parseJSON[CostAnalysis](resp.Text())
		if err != nil {
...
Provide your response in your directed JSON format.`,
		...
		messages := agentutil.BuildMessages(registry.ProcurementSpecialistCfg, prompt)

		resp, err := registry.ProcurementSpecialist.Chat(ctx, messages)
		if err != nil {
			return s, fmt.Errorf("procurement specialist failed: %w", err)
		}

		validation, err := parseJSON[ValidationResult](resp.Text())
		if err != nil {
...
Provide your response in your directed JSON format.`,
		...
		processor := func(ctx context.Context, task AnalysisTask) (AnalysisResult, error) {
			...
			case "budget":
				...
				messages := agentutil.BuildMessages(registry.BudgetAnalystCfg, task.Prompt)

				resp, err = registry.BudgetAnalyst.Chat(ctx, messages)
				if err != nil {
					return AnalysisResult{}, fmt.Errorf("budget analyst failed: %w", err)
				}
				validation, parseErr := parseJSON[BudgetValidation](resp.Text())
				if parseErr != nil {
			...
			case "optimizer":
				messages := agentutil.BuildMessages(registry.CostOptimizerCfg, task.Prompt)

				resp, err = registry.CostOptimizer.Chat(ctx, messages)
				if err != nil {
					return AnalysisResult{}, fmt.Errorf("cost optimizer failed: %w", err)
				}
				optimization, parseErr := parseJSON[CostOptimization](resp.Text())
				if parseErr != nil {
...
func createRoutingNode(wc *WorkflowConfig, registry *AgentRegistry) state.StateNode {
	return state.NewFunctionNode(func(ctx context.Context, s state.State) (state.State, error) {
		...
		if wc.SkipLegal {
			if cost < 200000 {
				return routeToExecutive(ctx, s, registry.ProgramDirector, registry.ProgramDirectorCfg, "Program Director", cost, "expedited")
			} else {
				return routeToExecutive(ctx, s, registry.DeputyDirector, registry.DeputyDirectorCfg, "Deputy Director", cost, "expedited")
			}
		}

		if cost < 50000 {
			return routeToExecutive(ctx, s, registry.ProgramDirector, registry.ProgramDirectorCfg, "Program Director", cost, "low-cost")
		}
		...
		if cost < 200000 {
			...
			return routeToExecutive(ctx, newState, registry.ProgramDirector, registry.ProgramDirectorCfg, "Program Director", cost, "standard-legal")
		}
		...
		return routeToExecutive(ctx, newState, registry.DeputyDirector, registry.DeputyDirectorCfg, "Deputy Director", cost, "full-security-review")
	})
}
...
Provide your response in your directed JSON format.`,
	...
	processor := func(ctx context.Context, task LegalTask) (LegalReview, error) {
		...
		reviewerCfg := registry.LegalReviewerCfgs[task.ReviewerIndex]

		messages := agentutil.BuildMessages(reviewerCfg, legalPrompt)

		resp, err := reviewer.Chat(ctx, messages)
		if err != nil {
			return LegalReview{}, fmt.Errorf("legal reviewer %d failed: %w", task.ReviewerIndex+1, err)
		}

		review, parseErr := parseJSON[LegalReview](resp.Text())
		if parseErr != nil {
...
Provide your response in your directed JSON format.`,
		...
		messages := agentutil.BuildMessages(registry.SecurityOfficerCfg, securityPrompt)

		resp, err := registry.SecurityOfficer.Chat(ctx, messages)
		if err != nil {
			return newState, fmt.Errorf("security officer failed: %w", err)
		}

		security, parseErr := parseJSON[SecurityReview](resp.Text())
		if parseErr != nil {
...
func routeToExecutive(ctx context.Context, s state.State, executive agent.Agent, executiveCfg *agentconfig.AgentConfig, title string, cost int, route string) (state.State, error) {
...
Provide justification for your decision.`,
	...
	messages := agentutil.BuildMessages(executiveCfg, prompt)

	resp, err := executive.Chat(ctx, messages)
	if err != nil {
		return s, fmt.Errorf("%s approval failed: %w", title, err)
	}

	decision, parseErr := parseJSON[ExecutiveDecision](resp.Text())
	if parseErr != nil {
