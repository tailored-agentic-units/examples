...
func main() {
	...
	baseConfig, err := agentconfig.LoadAgentConfig("orchestrate/phase-06-checkpointing/config.ollama.json")
	if err != nil {
		log.Fatalf("Failed to load config: %v", err)
	}

	baseConfig.Name = "data-analyst"
	baseConfig.SystemPrompt = `You are a scientific data analyst processing research data through multiple stages.
...
Keep responses to 1-2 sentences focusing on key findings or actions.`
	...
	dataAgent, err := agentutil.NewAgent(baseConfig)
	if err != nil {
