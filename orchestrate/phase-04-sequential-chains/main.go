...
func main() {
	...
	baseConfig, err := agentconfig.LoadAgentConfig("orchestrate/phase-04-sequential-chains/config.ollama.json")
	if err != nil {
		log.Fatalf("Failed to load config: %v", err)
	}

	baseConfig.Name = "research-analyst"
	baseConfig.SystemPrompt = `You are an expert research paper analyst.
...
Always respond in 1-2 sentences with specific details.`
	...
	analysisAgent, err := agentutil.NewAgent(baseConfig)
	if err != nil {
