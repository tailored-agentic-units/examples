...
func main() {
	...
	baseConfig, err := agentconfig.LoadAgentConfig("orchestrate/phase-02-03-state-graphs/config.ollama.json")
	if err != nil {
		log.Fatalf("Failed to load config: %v", err)
	}

	baseConfig.Name = "deployment-manager"
	baseConfig.SystemPrompt = `You are an expert DevOps deployment manager responsible for software deployments.
...
Always respond in 1-2 sentences with specific technical information.`
	...
	deploymentAgent, err := agentutil.NewAgent(baseConfig)
	if err != nil {
	...
	planNode := state.NewFunctionNode(func(ctx context.Context, s state.State) (state.State, error) {
		...
		messages := agentutil.BuildMessages(baseConfig, prompt)
	...
	deployNode := state.NewFunctionNode(func(ctx context.Context, s state.State) (state.State, error) {
		...
		deploymentConfirm := response.Text()
